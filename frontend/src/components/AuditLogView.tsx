...
import { Search, X, Filter, RefreshCw, ScrollText } from 'lucide-react';
...
export default function AuditLogView() {
  ...
  const loadLogs = () => {
    setLoading(true);
    activityLogApi.listMyCompany(FETCH_LIMIT)
  ...
  const filteredLogs = logs.filter((log) => {
    const matchesSearch = log.eventDescription.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  if (selectedLogId) {
    return <AuditLogDetailView logId={selectedLogId} onBack={() => setSelectedLogId(null)} showToast={showToast} />;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">History of signups, plan changes, support tickets, and exports for your workspace.</p>
        </div>
        <button
          onClick={loadLogs}
          disabled={loading}
      ...
      <div className="bg-white rounded-[20px] border border-gray-100 shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          {/* min-w is what lets the wrapper scroll: a w-full table can never be wider than its
              container, so instead of scrolling it crushed three columns until the timestamp broke
              across two lines and the status pill — an inline box — split into two half-pills.
              Description stays wrappable; it is prose. */}
          <table className="w-full min-w-[720px] text-left border-collapse text-xs">
            <thead>
              <tr className="border-b border-gray-100 bg-white text-gray-400 text-[10px] uppercase font-bold tracking-widest">
                ...
                <th className="px-6 py-4 whitespace-nowrap">Action Code</th>
            ...
            <tbody className="divide-y divide-gray-100 text-gray-700">
              {loading ? (
                <tr>
                  <td colSpan={3} className="text-center py-8 text-gray-400 text-xs font-medium">Loading audit log...</td>
                </tr>
              ) : filteredLogs.length === 0 ? (
                <tr>
                  <td colSpan={3} className="text-center py-12 text-gray-400 text-xs font-medium">
                ...
                paginatedLogs.map((log) => (
                  ...
                  >
                    <td className="px-6 py-4 font-mono text-[11px] text-gray-500 whitespace-nowrap">{log.timestamp}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-block px-2.5 py-0.5 rounded-full text-[10px] font-bold border ${log.badgeColor}`}>
                        {log.eventType}
                      </span>
                    </td>
